...
{
    ringbuf_t* rb = static_cast<ringbuf_t *>(malloc(sizeof(struct ringbuf_t)));
    if (rb) {

        /* One byte is used for detecting the full condition. */
        rb->size = capacity + 1;
        rb->buf = static_cast<uint8_t *>(malloc(rb->size));
        if (rb->buf)
            ringbuf_reset(rb);
        else {
            free(rb);
...
size_t
ringbuf_bytes_free__(const struct ringbuf_t *rb)
{
    if (rb->head >= rb->tail)
...
ringbuf_findchr(const struct ringbuf_t *rb, int c, size_t offset)
{
    ...
    const uint8_t *found = static_cast<const uint8_t *>(memchr(start, c, n));
    if (found)
...
ringbuf_read(int fd, ringbuf_t* rb, size_t count)
{
    ...
    ssize_t n = _read(fd, rb->head, count);
    if (n > 0) {
...
ringbuf_write(int fd, ringbuf_t* rb, size_t count)
{
    ...
    ssize_t n = _write(fd, rb->tail, count);
    if (n > 0) {
