...
{
   ScrRTSPServer::ScrRTSPServer(ScrH264Encoder * enc, int port, int httpPort)
      : fH264Encoder(enc), fPortNum(port), fHttpTunnelingPort(httpPort), fQuit(0), fBitRate(5120) //in kbs
   {
      fQuit = 0;
   };

   ScrRTSPServer::~ScrRTSPServer()
   {
   };

   void ScrRTSPServer::run()
   {
      ...
      if (rtspServer == nullptr)
      {
         *env << "LIVE555: Failed to create RTSP server: %s\n", env->getResultMsg();
         ...
         char const* descriptionString = "Combined Multiple Scrs Streaming Session";

         ScrFrameSource* source = ScrFrameSource::createNew(*env, fH264Encoder);
         ...
         ScrMediaSubsession* sub = ScrMediaSubsession::createNew(*env, inputDevice);
